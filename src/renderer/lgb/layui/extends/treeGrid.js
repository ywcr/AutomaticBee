  ...
  .define(["laytpl", "laypage", "layer", "form"], function (exports) {
    ...
    var $ = layui.$,
      ...
      table = {
        ...
        setDataList: function (tableId, list) {
          ...
          if (!table.cache[tableId].data.list)
            table.cache[tableId].data.list = [];
        ...
        setDataMap: function (tableId, map) {
          if (!table.cache[tableId]) table.cache[tableId] = {};
          if (!table.cache[tableId].data) table.cache[tableId].data = {};
          if (!table.cache[tableId].data.map)
            table.cache[tableId].data.map = {};
        ...
        setDataTreeList: function (tableId, treeList) {
          if (!table.cache[tableId]) table.cache[tableId] = {};
          if (!table.cache[tableId].data) table.cache[tableId].data = {};
          if (!table.cache[tableId].data.treeList)
            table.cache[tableId].data.treeList = {};
        ...
        setDataRootList: function (tableId, rootList) {
          if (!table.cache[tableId]) table.cache[tableId] = {};
          if (!table.cache[tableId].data) table.cache[tableId].data = {};
          if (!table.cache[tableId].data.upIds)
            table.cache[tableId].data.upIds = [];
        ...
        setCheckStatus: function (tableId, fildName, ids) {
          ...
          var that = table.getClass(tableId),
            ...
            arr = [],
            data = table.getDataList(tableId) || [],
            childs = that.layBody.find(
              'input[name="' + TABLE_CHECKBOX_ID + '"]'
            ); //复选框
          if (fildName && ids) {
            //设置选中
            ...
            idsarr.forEach(function (o) {
              ...
              if (temo) {
                ...
                that.layBody
                  .find(
                    'input[name="' + TABLE_CHECKBOX_ID + '"][value="' + v + '"]'
                  )
                  .prop("checked", true);
        ...
        radioStatus: function (tableId) {
          ...
          var nums = 0,
            ...
            data = table.getDataList(tableId) || [];
          var v = that.layBody
            .find("input[name='" + TABLE_RADIO_ID + "']:checked")
            .val();
          v = parseInt(v);
          data.forEach(function (e) {
        ...
        addRow: function (tableId, index, data) {
          ...
          var trs =
            '<tr data-index="' +
            data[table.config.indexName] +
            '"' +
            that.renderTrUpids(data) +
            ">" +
            tds.join("") +
            "</tr>";
          if (index == 0) {
        ...
        delRow: function (tableId, data) {
          ...
          sonList = options.isTree
            ? table.treeFindSonList(that.config.id, delDatas)
            : delDatas;
          ...
          for (var i = 0, len = list.length; i < len; i++) {
            ...
            sonList.forEach(function (temo) {
              if (
                temo[table.config.indexName] === list[i][table.config.indexName]
              ) {
                isP = false;
              }
            });
          ...
          table.setDataTreeList(
            tableId,
            that.resetDataTreeList(newList, table.getDataRootList(tableId))
          ); //处理树结构
          ...
          if (options.isPage)
            that.renderPage(
              that.config.page.count - Object.keys(delIds).length
            ); //分页渲染
          that.events(); //重新注册事件
        },
        /**
            ...
            if (temo.children.length > 0) {
              ...
              temSonList.forEach(function (temii) {
                if (!delIds[temii[table.config.indexName]]) {
                  sonList.push(temii);
                  delIds[temii[table.config.indexName]] =
                    temii[table.config.indexName];
                }
              });
            }
            sonList.push(temo);
    ...
    Class.prototype.resetDataTableList = function (treeList) {
      ...
      tableList.forEach(function (o) {
        ...
        if (
          !uo ||
          (uo[table.config.cols.isOpen] && uo[table.config.cols.isShow])
        ) {
          //没有父亲：显示；父亲打开状态（显示状态：显示；隐藏状态：隐藏）
    ...
    Class.prototype.treeConvertShowName = function (o) {
      ...
      var temhtml =
        '<div style="float: left;height: 28px;line-height: 28px;padding-left: ' +
        (function () {
          if (isTreeNode) {
            return "5px";
          } else {
        ...
        (function () {
          ...
          if (isTreeNode) {
            //父节点
            temTreeHtml =
              '<i class="layui-icon layui-tree-head">' +
              temTreeIsOpen +
              "</i>" +
    ...
    Class.prototype.treeNodeOpen = function (o, isOpen) {
      var that = this,
        tr = that.layBody.find(
          'tr[data-index="' + o[table.config.indexName] + '"]'
        );
      if (!o) {
        return;
      }
          ...
          for (var i = 0; i < temList.length; i++) {
            ...
            if (o[table.config.cols.isOpen]) {
              ...
              if (e[table.config.cols.isOpen] && e[table.config.cols.isShow]) {
                //该节点显示
                var temo = that.layBody.find(
                  'tr[data-index="' + n[table.config.indexName] + '"]'
                );
                temo.css("display", "");
    ...
    Class.prototype.treeIconRender = function (o, isHtml) {
      ...
      if (iconOpen) {
        ...
        if (isTreeNode) {
          ...
          if (o[table.config.cols.iconOpen] || o[table.config.cols.iconClose]) {
            ...
            temTreeHtml =
              ...
              o[options.idField] +
              " layui-tree-" +
              (o[table.config.cols.isOpen] ? "branch" : "leaf") +
              '" ' +
              iconOpen +
              ">" +
              (o[table.config.cols.isOpen]
                ? that.config.branch[1]
                : that.config.branch[0]) +
              "</i>";
          }
        } else {
          //叶子节点
        ...
        if (isHtml) {
          ...
          var temdiv = that.layBody
            .find('tr[data-index="' + o[table.config.indexName] + '"]')
            .find("td[data-field=" + options[TREE_SHOW_NAME] + "]")
            .find(".layui-table-cell");
          $(temdiv)
            .find("div .layui-tree-" + o[options.idField])
            .remove(); //节点附加div
    ...
    Class.prototype.pullData = function (curr, loadIndex) {
      ...
      if (options.url) {
        ...
        res[response.dataName] = options.data
          .concat()
          .splice(startLimit, options.limit);
        ...
        layui.each(item1, function (i2, item2) {
          ...
          if (item2.colspan > 1) {
            ...
            layui.each(cols[i1 + 1], function (i22, item22) {
              ...
              childIndex =
                childIndex + (item22.colspan > 1 ? item22.colspan : 1);
            });
          }
          ...
          layui.each(that.config.cols, function (i1, item1) {
            layui.each(item1, function (i2, item2) {
              ret +=
                ...
                "{" +
                "width:" +
                (item2.width ? item2.width + "px" : "0px") +
                "}";
            });
          });
          return ret;
      ...
      var td = [
        '<td data-field="' +
          ...
          (function () {
            ...
            if (cols.minWidth)
              attr.push('data-minwidth="' + cols.minWidth + '"'); //单元格最小宽度
            return attr.join(" ");
          })() +
          ">",
        '<div class="layui-table-cell laytable-cell-' +
          (function () {
            ...
            return cols.type === "normal"
              ? str
              : str + " laytable-cell-" + cols.type;
          })() +
          '">' +
          treeImgHtml +
          '<p style="width: auto;height: 100%;">' +
          (function () {
            ...
            if (cols.type === "checkbox") {
              return tplData[table.config.cols.cheDisabled]
                ...
                : '<input type="checkbox" name="' +
                    TABLE_CHECKBOX_ID +
                    '" value="' +
                    tplData[table.config.indexName] +
                    '" lay-skin="primary" ' +
                    (function () {
              ...
              //下拉框
              var rowsField = dl.ui.table.drop.findFieldObj(
                options.cols[0],
                field
              );
            ...
            return cols.templet
              ? (function () {
                  return typeof cols.templet === "function"
                    ...
                    : laytpl($(cols.templet).html() || String(content)).render(
                        tplData
                      );
                })()
              : content;
    ...
    Class.prototype.renderData = function (res, curr, count, sort) {
          ...
          layui.each(data, function (i1, obj) {
            ...
            trs.push(
              '<tr style="' +
                ...
                i1 +
                '" ' +
                that.renderTrUpids(obj) +
                ">" +
                tds.join("") +
                "</tr>"
            );
            trs_fixed.push(
              '<tr data-index="' + i1 + '">' + tds_fixed.join("") + "</tr>"
            );
            trs_fixed_r.push(
      ...
      //显示隐藏分页栏
      that.layPage[data.length === 0 && curr == 1 ? "addClass" : "removeClass"](
        HIDE
      );
      //排序
      if (sort) {
    ...
    Class.prototype.renderFilter = function () {
      ...
      v.push(
        '<table cellspacing="0" cellpadding="0" border="0" class="layui-table"><thead><tr>'
      );
      layui.each(options.cols, function (i, o) {
        layui.each(o, function (i2, item2) {
          var field = item2.field || i2;
          var minW = item2.minWidth
            ? "data-minwidth='" + item2.minWidth + "'"
            : "";
    ...
    Class.prototype.getColElem = function (parent, field) {
      ...
      return parent
        .eq(0)
        .find(".laytable-cell-" + (options.index + "-" + field) + ":eq(0)");
    };
    //渲染表单
    ...
    Class.prototype.loading = function () {
      ...
      if (options.loading && options.url) {
        return layer.msg("数据请求中", {
          icon: 16,
          offset: [
            that.elem.offset().top +
              that.elem.height() / 2 -
              35 -
              _WIN.scrollTop() +
              "px",
            that.elem.offset().left +
              that.elem.width() / 2 -
              90 -
              _WIN.scrollLeft() +
              "px",
    ...
    Class.prototype.syncCheckAll = function () {
      ...
      list.forEach(function (t) {
        if (!t[table.config.cols.cheDisabled]) {
          if (t[table.config.cols.isCheckName]) {
            var checkAllElem = that.layBody
              .find("tr[data-index=" + t[table.config.indexName] + "]")
              .find('input[name="' + TABLE_CHECKBOX_ID + '"]');
      ...
      if (temis) {
        //设置全选
        var checkAllElem = that.layHeader.find(
          'input[name="' + TABLE_CHECKBOX_ID + '"]'
        );
        checkAllElem.prop("checked", true);
      }
    ...
    Class.prototype.getCssRule = function (field, callback) {
      ...
      layui.each(rules, function (i, item) {
        if (
          item.selectorText ===
          ".laytable-cell-" + that.index + "-" + field
        ) {
    ...
    Class.prototype.columnWidthInit = function () {
      ...
      layui.each(options.cols, function (i1, item1) {
        layui.each(item1, function (i2, item2) {
          ...
          if (/\d+%$/.test(width)) {
            item2.width = width = Math.floor(
              (parseFloat(width) / 100) * cntrWidth
            );
      ...
      layui.each(options.cols, function (i1, item1) {
        layui.each(item1, function (i2, item2) {
          ...
          if (item2.colspan > 1) return;
          if (item2.width === 0) {
            item2.width = Math.floor(
              autoWidth >= minWidth ? autoWidth : minWidth
            ); //不能低于设定的最小宽度
          }
        });
      });
    ...
    Class.prototype.scrollPatch = function () {
      ...
      if (that.autoColNums && outWidth < 5 && !that.scrollPatchWStatus) {
        ...
        that.getCssRule(field, function (item) {
          ...
          item.style.width =
            parseFloat(width) - getScrollWidth - outWidth + "px";
          //二次校验，如果仍然出现横向滚动条
    ...
    Class.prototype.events = function () {
      ...
      bodytr.unbind("click").on("click", function (e) {
        ...
        var list = table.getDataList(that.config.id);
        var o = list[index];
        typeof options.onClickRow === "function" &&
          options.onClickRow(index, o);
      });
      //行双击事件
      bodytr.unbind("dblclick").on("dblclick", function (e) {
        ...
        var list = table.getDataList(that.config.id);
        var o = list[index];
        typeof options.onDblClickRow === "function" &&
          options.onDblClickRow(index, o);
      });
      //拖拽调整宽度
    ...
    Class.prototype.eventsinit = function () {
      ...
      that.layBody.on("click", "td div.layui-table-cell p", function () {
        ...
        if (!table.config.initWidth[c["type"]]) {
          if (elemCell.find(".layui-form-switch,.layui-form-checkbox")[0])
            return; //限制不出现更多（暂时）
          if (
            Math.round(elemCell.prop("scrollWidth")) >
            Math.round(elemCell.outerWidth())
          ) {
      ...
      that.elem.on(
        ...
        function () {
          ...
          typeof options.onCheck === "function" &&
            options.onCheck(obj, checked, isAll);
        }
      );
