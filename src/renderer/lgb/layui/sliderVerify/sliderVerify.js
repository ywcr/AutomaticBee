layui.define(["jquery", "layer", "form"], function (s) {
  var d = layui.jquery,
    ...
    h = {
      read: (function () {
        ...
        v.innerHTML = u;
        v.type = "text/css";
        (d("head link:last")[0] && d("head link:last").after(v)) ||
          d("head").append(v);
      })(),
    },
    m = function (u) {
    ...
    var x = d(
      [
        '<div id="' +
          u.domid +
          '"' +
          (u.isAutoVerify ? 'lay-verify="sliderVerify"' : "") +
          'class="slider-item">',
        '<div class="' + j + " " + u.bg + '"></div>',
        '<div class="' + q + '">' + u.text + "</div>",
        '<div class="' + i + ' layui-icon layui-icon-next"></div>',
      ].join("")
    );
    v.hide().after(x);
    u.domid = d("#" + u.domid);
  ...
  l.prototype.isMobile = function () {
    return (
      ...
      k.android ||
      k.ios ||
      (k.weixin && typeof k.weixin === Boolean)
    );
  };
  ...
  l.prototype.move = function (C, B) {
    ...
    if (z == u.distance) {
      ...
      var v = window.getComputedStyle
        ? window.getComputedStyle(u.bg, null)
        : u.bg.currentStyle;
